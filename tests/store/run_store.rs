...
fn run_store_persists_run_shards_and_events() {
    ...
    let loaded_events = load_events(&run_dir).expect("events should load");
    ...
    assert_eq!(loaded_events[0].timestamp, "2026-03-10T00:00:00Z");
    assert_eq!(loaded_events[0].shard_id.as_deref(), Some("01"));
    assert_eq!(loaded_events[0].message, "worker launched");
